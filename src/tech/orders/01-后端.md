...
```java
@Component
public class TaskHandlerStrategyContext {

    @Autowired
    private final Map<String, TaskHandler> taskHandlerStrategyMap = new ConcurrentHashMap<>();

    public TaskHandlerStrategyContext(Map<String, TaskHandler> strategyMap) {
        this.taskHandlerStrategyMap.clear();
        strategyMap.forEach((k, v) -> this.taskHandlerStrategyMap.put(k, v));
    }

    public TaskHandler getTaskHandlerByTaskType(TaskTypeEnum taskTypeEnum) {
        // 流程类型为空，默认为普通流程
        if (null == taskTypeEnum || taskTypeEnum == TaskTypeEnum.BUSINESS) {
            return getNormalTaskHandlerByTaskType();
        }
        return taskHandlerStrategyMap.get(taskTypeEnum.getCode());
    }

    public TaskHandler getNormalTaskHandlerByTaskType() {
        return taskHandlerStrategyMap.get(TaskTypeEnum.NORMAL.getCode());
    }
}
```

实现类如下，`Spring会自动装配兼容类型的Bean，Bean名称作为键`

```java
@Component("APPROVAL")
@Slf4j
public class ApprovalTaskHandlerImpl implements TaskHandler {}

@Component("NORMAL")
@Slf4j
public class NormalTaskHandlerImpl implements TaskHandler {}
```
