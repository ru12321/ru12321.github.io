...
* 每一个服务的名称至关重要，服务名称必需唯一，服务名称出现下划线

* 微服务的每个服务都是集群部署

* 服务注册中心组件：服务发现、服务注册、健康检查、存储服务元数据
...
![image-20200708224716035](./springcloud/image-20200708224716035.png)
...
![image-20200723155352063](./springcloud/image-20200723155352063.png)
...
![image-20200318082336122](./springcloud/image-20200318082336122.png)
...
![image-20200724143456045](./springcloud/image-20200724143456045.png)
...
![image-20200724161314786](./springcloud/image-20200724161314786.png)
...
```markdown
# 1.翻译
- springcloud是一个由众多独立子项目组成的大型综合项目，原则每个子项目上有不同的发布节奏,都维护自己发布版本号。为了更好的管理springcloud的版本,通过一个资源清单BOM(Bill of Materials),为避免与子项目的发布号混淆，所以没有采用版本号的方式，而是通过命名的方式。这些名字是按字母顺序（A-Z）排列的。如伦敦地铁站的名称（“天使”是第一个版本，“布里斯顿”是第二个版本,"卡姆登"是第三个版本）。当单个项目的点发布累积到一个临界量，或者其中一个项目中有一个关键缺陷需要每个人都可以使用时，发布序列将推出名称以“.SRX”结尾的“服务发布”，其中“X”是一个数字。
...
* **注意springcloud和springboot的版本匹配关系**
...
![image-20200709112427684](./springcloud/image-20200709112427684.png)
...
![image-20200709115802270](./springcloud/image-20200709115802270.png)
...
![image-20200709120120478](./springcloud/image-20200709120120478.png)

![image-20200709120209047](./springcloud/image-20200709120209047.png)
...
![image-20200709124952525](./springcloud/image-20200709124952525.png)
...
springcloud支持的多种注册中心Eureka、Consul、Zookeeper（java 无ui）、以及阿里巴巴推出Nacos。这些注册中心在本质上都是用来管理服务的注册和发现以及服务状态的检查的。
...
- Eureka是Netflix开发的服务发现框架，本身是一个基于RESTful的服务。SpringCloud将它集成在其子项目spring-cloud-netflix中，以实现SpringCloud的服务注册和发现功能。
	Eureka包含两个组件：Eureka Server和Eureka Client。
```

**Eureka Server启动的时候自己也是一个客户端**
...
![image-20200709161916871](./springcloud/image-20200709161916871.png)

```markdown
# 5.虽然能看到管理界面为什么项目启动控制台报错?
```

![image-20200709162307608](./springcloud/image-20200709162307608.png)
...
![image-20200709164622017](./springcloud/image-20200709164622017.png)

```markdown
# 5.查看eureka server的服务注册情况
```

![image-20200709164729870](./springcloud/image-20200709164729870.png)

##### eureka自我保护机制

![image-20221026161310544](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221026161310544.png)
...
![image-20200709171532408](./springcloud/image-20200709171532408.png)
...
eureka.server.eviction-interval-timer-in-ms=3000 #关闭后，也有超时时间。超时3s自动清除
```

![image-20200709231727148](./springcloud/image-20200709231727148.png)

```markdown
# 3.在eureka client端减短服务心跳的时间
```
...
# 原有默认计算方式：90s为1组，30s发一次，假如是15分钟一个周期，如果挂掉15 * 60 / 90 * 3 * 85%这个数量30*0.85 = 25.5个，那么移除该客户端服务
# 总之是统计心跳失败比例>85%，就移除该客户端服务
```
...
![image-20200709232933894](./springcloud/image-20200709232933894.png)
...
![image-20200709233215860](./springcloud/image-20200709233215860.png)

不推荐：eureka 必须**手动 代码形式** 开发服务注册中心



##### eureka server集群

https://www.bilibili.com/video/BV1S5411c7hM/?p=9&spm_id_from=pageDriver&vd_source=c7a3b89462edde12dbd26250fc0f01ee

多个server注册中心相互注册和发现，高可用处理

![image-20221026164357138](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221026164357138.png)

方式：

![image-20221026164431836](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221026164431836.png)

* 服务端：8761向8762、8763两个注册中心注册，其它两个同理；

![image-20221026164631888](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221026164631888.png)

* 客户端：
  * 一开启，三个注册中心都能有注册信息
  * 注册地址尽量写多个

![image-20221026165113798](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221026165113798.png)

* 客户端集群：多个相同应用开在不同端口，名称一样；这样在服务端看的话，客户端就是列表

![image-20221026165422128](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221026165422128.png)
...
![image-20200710104357091](./springcloud/image-20200710104357091.png)
...
![image-20200710105007805](./springcloud/image-20200710105007805.png)
...
![image-20200710105912943](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200710105912943.png)

* 管理界面

dcl：代表数据中心名称 datacenter 默认为dcl

services：代表consul中服务的注册服务列表，默认是client server同时启动自己注册自己

nodes：consul如果是集群，就可以查看集群节点

key/value：可以存储key--value数据

ACL：版本控制

##### 开发consul 客户端即微服务

spring团队提供了客户款依赖，将指定服务注册到consul服务中心
...
![image-20200710113855944](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200710113855944.png)
...
```markdown
# 3.加注解@EnableDiscoveryClient（通用服务客户端注解 可以代表nacaos,zk,consul），启动服务查看consul界面服务信息
```
...
![image-20221027105145604](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221027105145604.png)
...
![image-20221027105435042](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20221027105435042.png)
...
![image-20200710135837525](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200710135837525.png)
...
方式1：Http rest方式，使用Http协议， JSON数据。**效率低，但低耦合**

方式2：RPC方式，远程过程调用，二进制序列化数据。**效率高，不能低耦合**

OSI 物理层、数据链路层、网络层、传输层（RPC）、会话层、表示层、应用层（http）



* 使用RPC框架Dubbo（java编写）要求两边系统使用的都是java语言
...
spring:
  ...
  cloud:
    consul:
      host: localhost #consul服务中心地址
      port: 8500 #consul服务中心端口
------------------------------------------
server:
  port: 8888
...
![image-20200713140804414](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713140804414.png)
...
![image-20200713162940968](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713162940968.png)
...
![image-20200713163722927](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713163722927.png)
...
![image-20200713195706787](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713195706787.png)
...
![image-20200713202802056](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713202802056.png)
...
![image-20200713205125242](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713205125242.png)
...
![image-20200713205734920](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713205734920.png)
...
![image-20200713205823467](./springcloud/image-20200713205823467.png)

```markdown
# 测试访问
```

![image-20200713205919054](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713205919054.png)

![image-20200713210001477](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713210001477.png)
...
![image-20200713210437488](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713210437488.png)
...
![image-20200713210641668](./springcloud/image-20200713210641668.png)
...
![image-20200713211213241](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713211213241.png)
...
![image-20200713211308524](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713211308524.png)

```markdown
# 测试
```

![image-20200713211338475](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713211338475.png)

![image-20200713211402844](./springcloud/image-20200713211402844.png)
...
![image-20200713213322984](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713213322984.png)

```markdown
# 2.进行客户端调用
```

![image-20200713213415230](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713213415230.png)
...
![image-20200713215108861](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200713215108861.png)
...
![image-20200715123359665](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200715123359665.png)
...
根本原因：在调用链路中链路中某一服务因为执行业务时间过长，或者是大规模出现异常导致自身服务不可用，并把这种不可用放大
...
![image-20200715151728240](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200715151728240.png)
...
![image-20200715152623313](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200715152623313.png)

**每个请求都是tomcat启动的线程，当不断有请求线程没有被释放，线程资源越来越多，服务就会不可用**

#### 2.服务熔断--防止服务雪崩现象出现

熔断机制：每个微服务都**引入自己服务的监控器**Hystrix

如果没有服务熔断：所有的B请求都在等待着C服务的响应，消耗B线程资源，会导致B服务都不可用，从而雪崩~~
...
![image-20200717085946385](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200717085946385.png)
...
![image-20200717112327729](https://picbed-for-mrru-mdfile.oss-cn-chengdu.aliyuncs.com/mrru-glodon/image-20200717112327729.png)
...
**hystrix依赖更新**https://blog.csdn.net/qq_45593903/article/details/123945035

```txt
@EnableCircuitBreaker注解的文档已经明确表明，从3.0.1版本开始Hystrix已经从SpringCloudNetfix中移除，并且该注解已过时。
现用@EnableHystrix注解即可
如果需要使用@EnableHystrix注解，则需在父工程中显式的指明hystrix版本号
```
...
![image-20200717090841831](./springcloud/image-20200717090841831.png)

![image-20200717091028876](./springcloud/image-20200717091028876.png)
...
**一旦断路器打开后，到该服务的所有请求都不可用，直到关闭断路器才又可用**
...
![image-20200717092819616](./springcloud/image-20200717092819616.png)

​	

整个流程:当Hvstrix监控到对该服务接口调用触发1 2 两个烟时会在系统中自动发断器在断器打开期间内任何到该接口请求均不可用,同时在断路器打开**5s后断路器会处于半开状态**，此时断路器允许放行一个请求到该服务接口,如果该请求执行成功,断路器彻底关闭,如果该请求执行失败断路器重新打开。

---

情况1：服务提供方报错了，服务提供方需要服务熔断处理，给出友好提示；

情况2：如果提供方整个挂了，服务调用方已经调用不了，这时也需要熔断处理，给上层feign一个友好的提示；
...
![image-20200716101101091](./springcloud/image-20200716101101091.png)
...
![image-20200717101921108](./springcloud/image-20200717101921108.png)
...
![image-20200716161556743](./springcloud/image-20200716161556743.png)
...
![image-20200717154912206](./springcloud/image-20200717154912206.png)
...
![image-20200717155059512](./springcloud/image-20200717155059512.png)
...
![image-20200717155120335](./springcloud/image-20200717155120335.png)

```markdown
# 5.通过监控界面监控
```

![image-20200717155258994](./springcloud/image-20200717155258994.png)

```markdown
# 6.点击监控,一致loading,打开控制台发现报错[特别坑]
```

![image-20200717155555786](./springcloud/image-20200717155555786.png)
...
![image-20200717160636218](./springcloud/image-20200717160636218.png)

#### 8.Hystrix停止维护

![image-20200717161223806](./springcloud/image-20200717161223806.png)

![image-20200717161400285](./springcloud/image-20200717161400285.png)
...
b.网关可以实现请求路由发(router dispatcher) 以及请求过程负载均衡
...
![image-20200720171205828](./springcloud/image-20200720171205828.png)
...
# 1.创建项目引入网关依赖  注意要去掉springweb的依赖
```
...
![image-20200720175051402](./springcloud/image-20200720175051402.png)
...
![image-20200720212535357](./springcloud/image-20200720212535357.png)

```markdown
- 在启动日志中发现,gateway为了效率使用webflux进行异步非阻塞模型的实现,因此和原来的web包冲突,去掉原来的web即可
```

![image-20200720212653494](./springcloud/image-20200720212653494.png)

```markdown
- 再次启动成功启动
```

![image-20200720213657788](./springcloud/image-20200720213657788.png)
...
![image-20200721103141491](./springcloud/image-20200721103141491.png)
...
![image-20200720220647899](./springcloud/image-20200720220647899.png)
...
![image-20200721110013966](./springcloud/image-20200721110013966.png)

![image-20200721110040104](./springcloud/image-20200721110040104.png)
...
![image-20200721112751340](./springcloud/image-20200721112751340.png)
...
![image-20200721152720455](./springcloud/image-20200721152720455.png)
...
![image-20200721161002001](./springcloud/image-20200721161002001.png)

![image-20200721161421845](./springcloud/image-20200721161421845.png)

```markdown
# 2.使用内置过滤器
```

![image-20200721152425733](./springcloud/image-20200721152425733.png)
...
![image-20200721164304994](./springcloud/image-20200721164304994.png)
...
![image-20200721180134903](./springcloud/image-20200721180134903.png)
...
![image-20200721182003376](./springcloud/image-20200721182003376.png)
...
![image-20200721182105648](./springcloud/image-20200721182105648.png)
...
![image-20200721182142000](./springcloud/image-20200721182142000.png)
...
![image-20200721183541178](./springcloud/image-20200721183541178.png)
...
![image-20200721183727767](./springcloud/image-20200721183727767.png)
...
![image-20200721221656436](./springcloud/image-20200721221656436.png)
...
![image-20200721221951670](./springcloud/image-20200721221951670.png)
...
![image-20200722105313716](./springcloud/image-20200722105313716.png)
...
![image-20200722105950808](./springcloud/image-20200722105950808.png)
...
![image-20200722102322149](./springcloud/image-20200722102322149.png)
...
![image-20200722102851999](./springcloud/image-20200722102851999.png)![image-20200722102901146](./springcloud/image-20200722102901146.png)
...
![image-20200722103435260](./springcloud/image-20200722103435260.png)
...
![image-20200722103823678](./springcloud/image-20200722103823678.png)

```markdown
# 再次启动服务
```

![image-20200722103913142](./springcloud/image-20200722103913142.png)

![image-20200722104031932](./springcloud/image-20200722104031932.png)
...
![image-20200722144351017](./springcloud/image-20200722144351017.png)
...
![image-20200722153537692](./springcloud/image-20200722153537692.png)

```markdown
# 4.在远程配置中加入name并启动测试
```

![image-20200722153731602](./springcloud/image-20200722153731602.png)

```markdown
# 5.启动之后直接访问
```

![image-20200722153806932](./springcloud/image-20200722153806932.png)

```markdown
# 6.修改远程配置
```

![image-20200722203225968](./springcloud/image-20200722203225968.png)
...
![image-20200722203317795](./springcloud/image-20200722203317795.png)
...
![image-20200722203417879](./springcloud/image-20200722203417879.png)

```markdown
# 9.在次访问发现配置已经成功刷新
```

![image-20200722203452506](./springcloud/image-20200722203452506.png)
...
![image-20200723150335451](./springcloud/image-20200723150335451.png)
...
![image-20190925220343521](./springcloud/image-20190925220343521.png)
...
![image-20190925222230260](./springcloud/image-20190925222230260-3836271.png)

将上图中配置文件中红色部分去掉`%%`,以及最后的`,`逗号 修改为下图:

![image-20190925222329200](./springcloud/image-20190925222329200-3836312.png)
...
![image-20190925222743776](./springcloud/image-20190925222743776-3836511.png)
...
```
 ...
 ![image-20190926194738708](./springcloud/image-20190926194738708-3836601.png)
...
![image-20190926194954822](./springcloud/image-20190926194954822-3836665.png)
...
![image-20200723104333906](./springcloud/image-20200723104333906.png)
...
![image-20200723105645915](./springcloud/image-20200723105645915.png)
...
![image-20200723111220198](./springcloud/image-20200723111220198.png)
...
![image-20200723111312496](./springcloud/image-20200723111312496.png)

```properties
spring.cloud.config.fail-fast=true
```

![image-20200723111754187](./springcloud/image-20200723111754187.png)
...
![image-20200723112316476](./springcloud/image-20200723112316476.png)
...
![image-20200723120419412](./springcloud/image-20200723120419412.png)

![image-20200723120947229](./springcloud/image-20200723120947229.png)
...
![image-20200723121203864](./springcloud/image-20200723121203864.png)
